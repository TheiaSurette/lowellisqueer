      ...
      <div className="mx-auto max-w-[1100px] px-8">
        ...
        <form onSubmit={handleSubmit} className="flex gap-3 max-sm:flex-col">
          ...
          <button
            type="submit"
            disabled={isPending}
            className="inline-flex items-center justify-center gap-2 border-2 border-primary bg-primary px-6 py-2.5 text-xs font-medium tracking-wider text-primary-foreground uppercase transition-colors hover:bg-primary/90 disabled:opacity-50"
          >
        ...
        {message && (
          <p
            className={`mt-3 text-sm font-light ${isError ? "text-destructive" : "text-foreground"}`}
          >
            {message}
          </p>
        )}
