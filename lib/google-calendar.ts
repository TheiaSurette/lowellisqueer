...
export function extractInstagramUrl(description: string): string | null {
  const match = description.match(
    /(?:<a[^>]*>)?(https?:\/\/(?:www\.)?instagram\.com\/(?:p|reel)\/[A-Za-z0-9_-]+\/?)[^\s<]*(?:<\/a>)?/i
  )
...
export function getEmbedUrl(mode: "MONTH" | "AGENDA" = "MONTH"): string {
  const calendarId = process.env.NEXT_PUBLIC_GOOGLE_CALENDAR_ID ?? ""
  return `https://calendar.google.com/calendar/embed?src=${encodeURIComponent(calendarId)}&ctz=${TZ}&mode=${mode}`
}
